...
const apiRequest = async <T>(endpoint = '', options: RequestInit = {}): Promise<ApiResponse<T>> => {
  if (!BASE_URL) {
    throw new ApiError('API Base URL is not defined. Please check your configuration.');
  }
...
export const doctorApi = {
  ...
  getAllDoctors: async (query?: string): Promise<Doctor[]> => {
    try {
      console.log(`Fetching doctors${query ? ` with query: "${query}"` : ''}...`);
      ...
      if (error instanceof ApiError) {
        throw new ApiError(`Failed to fetch doctors: ${error.message}`, error.status, error.code);
      }
      throw error;
    }
  },
  ...
  getDoctorById: async (doctorId: string): Promise<Doctor> => {
    ...
    try {
      ...
      if (error instanceof ApiError) {
        throw new ApiError(`Failed to fetch doctor: ${error.message}`, error.status, error.code);
      }
      throw error;
    }
  },
