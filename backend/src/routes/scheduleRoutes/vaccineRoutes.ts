...
const updateVaccineValidation = [
  param("id").isMongoId().withMessage("Invalid vaccine ID format"),
  body("name")
    .optional()
    .isLength({ max: 100 })
    .withMessage("Vaccine name cannot exceed 100 characters")
...
router.get(
  ...
  idValidation,
  validateRequest,
  VaccinationController.getVaccineById
);
...
router.get(
  ...
  vaccineIdValidation,
  validateRequest,
  VaccinationController.getVaccineByVaccineId
);
...
router.delete(
  ...
  idValidation,
  validateRequest,
  VaccinationController.deleteVaccine
);
