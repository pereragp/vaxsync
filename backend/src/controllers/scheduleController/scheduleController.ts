...
export class ScheduleController {
  ...
  static async createVaccineSchedule(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      ...
      const vaccinationRecord = new VaccinationRecord({
        ...
        doses: Array.from({ length: totalDoses }, (_, index) => ({
          ...
          dateScheduled: new Date(
            startDate.getTime() + index * interval * 24 * 60 * 60 * 1000
          ),
          status: "scheduled",
        })),
      });
  ...
  static async getAllVaccineSchedules(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      ...
      const schedules = await VaccinationRecord.find(filter)
        ...
        .populate(
          "dependentIds",
          "firstName lastName dateOfBirth gender dependentType"
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limitNum)
  ...
  static async updateVaccineSchedule(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      const { scheduleId } = req.params;
      const { vaccineName, healthcareProvider, notes, scheduleDate, interval } =
        req.body;
      ...
      if (scheduleDate || interval !== undefined) {
        const startDate = scheduleDate
          ...
          : new Date(existingSchedule.doses[0].dateScheduled);
        const intervalDays =
          interval !== undefined ? interval : existingSchedule.interval;

        // Recalculate all dose dates
        updateData.doses = existingSchedule.doses.map(
      ...
      if (updateData.overallStatus === "completed") {
        console.log(
          `Schedule ${scheduleId} marked as completed, triggering health card sync`
        );
  ...
  static async updateDoseStatus(req: Request, res: Response): Promise<void> {
    try {
      ...
      const allCompleted = schedule.doses.every(
        (dose) => dose.status === "completed"
      );
      const anyCancelled = schedule.doses.some(
      ...
      if (status === "completed") {
        console.log(
          `Dose ${doseNumber} completed, triggering health card sync for schedule ${scheduleId}`
        );
        await syncVaccinesToHealthCard(scheduleId);
      }
  ...
  static async deleteVaccineSchedule(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
